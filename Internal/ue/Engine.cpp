...
bool Engine::Init()
{
	...
	GObjects = reinterpret_cast<decltype(GObjects)>(Memory::FindPointer(main, objSig, sizeof(objSig), 0));
	...
	GNames = reinterpret_cast<decltype(GNames)>(Memory::FindPointer(main, poolSig, sizeof(poolSig), 0));
	...
	GWorld = reinterpret_cast<decltype(GWorld)>(Memory::FindPointer(main, worldSig, sizeof(worldSig), 0));
